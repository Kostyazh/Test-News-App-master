...
export class SearchedNewsComponent implements OnInit {
  ...
  loadnews(search){
    this.apicall = true;
    this.news = [];
      this.apiCalls.getNewsApiSearch(search, '2020-07-23',null,this.pagecount).then((response: HttpResponse<any>) => {
        this.title = search == 'a'? "Home" : this.toTitleCase(search);
        this.apicall = false;
        if (response.status == 200) {
          // console.log(response.body);
          response.body.articles.forEach(article => {
            article.publishedAt = moment(article.publishedAt).fromNow();//time
